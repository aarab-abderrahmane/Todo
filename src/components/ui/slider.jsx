    ...
    <SliderPrimitive.Root
      ...
      <SliderPrimitive.Track
        ...
        <SliderPrimitive.Range
          className="absolute bg-[var(--color-text)] data-[orientation=horizontal]:h-full data-[orientation=vertical]:w-full rounded-full bg-primary z-0" />
      </SliderPrimitive.Track>
      {Array.from({ length: _values.length }, (_, index) => (
        <SliderPrimitive.Thumb
          ...
          className={cn(
            ...
            showGlass
              ? "scale-105 bg-white/8 backdrop-blur-lg backdrop-brightness-100  border-[var(--color-text)] border-2 shadow-[0_8px_32px_0_rgba(31,38,135,0.25)] ring-2 ring-foreground/15 bg-[var(--color-button)]"
              : "bg-white/50 border-2 border-[var(--color-text)] shadow-sm"
          )}>
          {showGlass && (
            <div
              className="absolute top-0.5 left-1 right-1 h-0.5 rounded-full  pointer-events-none opacity-60 bg-gradient-to-r from-white/60 via-white/30 to-white/60  " />
          )}
        </SliderPrimitive.Thumb>
      ))}
    </SliderPrimitive.Root>
